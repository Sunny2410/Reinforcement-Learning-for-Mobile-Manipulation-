...
class BaseController:
    def __init__(self, joints_base: List[str], kinematics, Kp: float = 300):
        """
        Initialize base controller.
    ...
    def update_control_loop(self) -> Dict[str, np.ndarray]:
        ...
        pos_error = self._target_pos - curr_pos
        yaw_error = (self._target_yaw - curr_yaw + np.pi) % (2 * np.pi) - np.pi

        pos_dist = np.linalg.norm(pos_error)
        yaw_dist = abs(yaw_error)
        # -------------------------------
        # Nếu đây là TURN action
        # -------------------------------
        if self._turning_action:
            if yaw_dist > self.yaw_threshold:
                # Còn sai số yaw → xoay tiếp
                omega = self.Kp * yaw_error
                omega = np.clip(omega, -self._max_vel_angular, self._max_vel_angular)
                wheel_cmd = self.kinematics.compute_wheel_speeds_yaw(
                    target_rel_yaw=omega,
                    current_rel_yaw=0.0,
                    k=1.0
                )
                ...
                # Đã xoay xong → bù sai số pos 1 lần
                if pos_dist > self.pos_threshold:
                    ...
                    v_x = self.Kp * rel_error[0]
                    v_y = self.Kp * rel_error[1]

                    v_x = np.clip(v_x, -self._max_vel_linear, self._max_vel_linear)
                    v_y = np.clip(v_y, -self._max_vel_linear, self._max_vel_linear)

                    wheel_cmd = self.kinematics.compute_wheel_speeds_position(
                        target_rel_pos=np.array([v_x, v_y]),
                        current_rel_pos=np.zeros(2),
                        k=0.01
                    )
                    # Sau khi bù thì dừng hẳn
                    self._is_moving = False
                    return {"base_qvel": wheel_cmd}

                # Nếu pos cũng ổn rồi thì stop
                print("TURN action done, base stabilized.")
                self._is_moving = False
                return {"base_qvel": np.zeros(len(self.joints_base))}
